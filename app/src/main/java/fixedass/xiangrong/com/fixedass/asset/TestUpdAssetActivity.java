...
import fixedass.xiangrong.com.fixedass.manage.TestUpdActivity;
...
/*添加资产的资产详情*/
public class TestUpdAssetActivity extends Activity implements View.OnClickListener{
    /*标题栏*/
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        /*读取数据*/
        SharedPreferences sharedPreferences = TestUpdAssetActivity.super.getSharedPreferences("ipConfig", Activity.MODE_PRIVATE);
    ...
    private void getAddAss(){
        if (!Redirect.checkNetwork(TestUpdAssetActivity.this)) {
            Toast toast = Toast.makeText(TestUpdAssetActivity.this,"网络未连接", Toast.LENGTH_SHORT);
            toast.setGravity(Gravity.CENTER, 0, 0);
            toast.show();
        }else{
            // 提示框
            dialog = new ProgressDialog(TestUpdAssetActivity.this);
            dialog.setTitle("提示");
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btnBack:
                Redirect.redirect(TestUpdAssetActivity.this, TestUpdActivity.class, bundle);
                break;
            case R.id.btnQuit:      //保存
                ...
                if(stoLists.size()==0){
                    Dialog dialog = new AlertDialog.Builder(TestUpdAssetActivity.this)
                            .setTitle("资产维修提示")
    ...
    View.OnKeyListener returnKey = new View.OnKeyListener() {
        ...
        public boolean onKey(View v, int keyCode, KeyEvent event) {
            if (keyCode == KeyEvent.KEYCODE_ENTER && event.getAction() == KeyEvent.ACTION_DOWN) {
                Redirect.hideIM(v,TestUpdAssetActivity.this);

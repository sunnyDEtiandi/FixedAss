...
import fixedass.xiangrong.com.fixedass.asset.TestAssetActivity;
...
public class TestAddActivity extends Activity implements View.OnClickListener {
...
//    private ProgressDialog dialog;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        /*读取数据*/
        SharedPreferences sharedPreferences = TestAddActivity.super.getSharedPreferences("ipConfig", Activity.MODE_PRIVATE);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()){
            case R.id.btnBack:          //返回
                Redirect.redirect(TestAddActivity.this, TestActivity.class, bundle);
                break;
            case R.id.btnQuit:          //保存
                if (stoList.size()<1){
                    Dialog dialog = new AlertDialog.Builder(TestAddActivity.this)
                            .setTitle("资产检验提示")
                ...
                System.out.println("========="+testInfoList.size());
                ...
                String conSql = "createDate="+createDateStr+"&operNote="+operNoteStr+"&barCode="+barCode+"&testList="+testListJson;
                Log.i("conSql",conSql);
                save(conSql);
                break;
            case R.id.addAsset:                 //添加资产
                setBundle(bundle);
                Redirect.redirect(TestAddActivity.this, TestAssetActivity.class, bundle);
                Redirect.hideIM(v, TestAddActivity.this);
                break;
            case R.id.scanAsset:                //扫描资产
//                Toast.makeText(TestAddActivity.this, "扫描资产", Toast.LENGTH_SHORT).show();
                ActivityCompat.requestPermissions(TestAddActivity.this,
                        new String[]{Manifest.permission.CAMERA,Manifest.permission.WRITE_EXTERNAL_STORAGE},
                        1);
                Intent intent = new Intent(TestAddActivity.this, CaptureActivity.class);
            ...
            case R.id.createDate:
                Redirect.hideIM(v, TestAddActivity.this);
                Dialog dialog = new DatePickerDialog(TestAddActivity.this, new DatePickerDialog.OnDateSetListener(){
                    @Override
    ...
    private void save(String conSql){
        if (!Redirect.checkNetwork(TestAddActivity.this)) {
            Toast toast = Toast.makeText(TestAddActivity.this,"网络未连接", Toast.LENGTH_SHORT);
    ...
    public class MySaveThread implements Runnable {
        ...
        public void run() {
            handler.post(new Runnable() {
                /*所有的数据*/
                String path = "http://" + ipStr + "/FixedAssService/test/add";
                ...
                public void run() {
                    ...
                    if (num>0){
                        Toast.makeText(TestAddActivity.this,"检验单创建成功",Toast.LENGTH_SHORT).show();
                        Bundle bundle = new Bundle();
                        bundle.putInt("fragid",fragid);
                        bundle.putSerializable("user",user);
                        Redirect.redirect(TestAddActivity.this, TestActivity.class, bundle);
                    }else {
                        Toast.makeText(TestAddActivity.this,"检验单创建失败，请重新创建检验单",Toast.LENGTH_LONG).show();
                    }
                }
            });
//            dialog.dismiss();

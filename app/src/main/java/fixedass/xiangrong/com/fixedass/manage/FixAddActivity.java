...
import fixedass.xiangrong.com.fixedass.asset.FixAssetActivity;
...
public class FixAddActivity extends Activity implements View.OnClickListener {
...
//    private ProgressDialog dialog;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_fix_add);

        ExitApplication.getInstance().addActivity(this);

        /*读取数据*/
        SharedPreferences sharedPreferences = FixAddActivity.super.getSharedPreferences("ipConfig", Activity.MODE_PRIVATE);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()){
            case R.id.btnBack:          //返回
                Redirect.redirect(FixAddActivity.this, FixActivity.class, bundle);
                break;
            case R.id.btnQuit:          //保存
                if (stoList.size()<1){
                    Dialog dialog = new AlertDialog.Builder(FixAddActivity.this)
                            .setTitle("资产维修提示")
                ...
                System.out.println("========="+fixInfoList.size());
                ...
                String conSql = "createDate="+createDateStr+"&operNote="+operNoteStr+"&barCode="+barCode+"&fixList="+fixListJson;
                Log.i("conSql",conSql);
                save(conSql);
                break;
            case R.id.addAsset:                 //添加资产
                setBundle(bundle);
                Redirect.redirect(FixAddActivity.this, FixAssetActivity.class, bundle);
                Redirect.hideIM(v, FixAddActivity.this);
                break;
            case R.id.scanAsset:                //扫描资产
//                Toast.makeText(FixAddActivity.this, "扫描资产", Toast.LENGTH_SHORT).show();
                ActivityCompat.requestPermissions(FixAddActivity.this,
                        new String[]{Manifest.permission.CAMERA,Manifest.permission.WRITE_EXTERNAL_STORAGE},
                        1);
                Intent intent = new Intent(FixAddActivity.this, CaptureActivity.class);
            ...
            case R.id.createDate:
                Redirect.hideIM(v, FixAddActivity.this);
                Dialog dialog = new DatePickerDialog(FixAddActivity.this, new DatePickerDialog.OnDateSetListener(){
                    @Override
    ...
    private void save(String conSql){
        if (!Redirect.checkNetwork(FixAddActivity.this)) {
            Toast toast = Toast.makeText(FixAddActivity.this,"网络未连接", Toast.LENGTH_SHORT);
            toast.setGravity(Gravity.CENTER, 0, 0);
            toast.show();
        }else{
            // 提示框
            /*dialog = new ProgressDialog(FixAddActivity.this);
            dialog.setTitle("提示");
    ...
    public class MySaveThread implements Runnable {
        ...
        public void run() {
            handler.post(new Runnable() {
                /*所有的数据*/
                String path = "http://" + ipStr + "/FixedAssService/fix/add";
                ...
                public void run() {
                    ...
                    if (num>0){
                        Toast.makeText(FixAddActivity.this,"维修单创建成功",Toast.LENGTH_SHORT).show();
                        Bundle bundle = new Bundle();
                        bundle.putInt("fragid",fragid);
                        bundle.putSerializable("user",user);
                        Redirect.redirect(FixAddActivity.this, FixActivity.class, bundle);
                    }else {
                        Toast.makeText(FixAddActivity.this,"维修单创建失败，请重新创建维修单",Toast.LENGTH_LONG).show();
                    }
                }
            });
//            dialog.dismiss();

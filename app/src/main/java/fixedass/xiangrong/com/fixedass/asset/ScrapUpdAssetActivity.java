...
import fixedass.xiangrong.com.fixedass.manage.ScrapUpdActivity;
...
/*添加资产的资产详情*/
public class ScrapUpdAssetActivity extends Activity implements View.OnClickListener{
    /*标题栏*/
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        /*读取数据*/
        SharedPreferences sharedPreferences = ScrapUpdAssetActivity.super.getSharedPreferences("ipConfig", Activity.MODE_PRIVATE);
    ...
    private void getAddAss(){
        if (!Redirect.checkNetwork(ScrapUpdAssetActivity.this)) {
            Toast toast = Toast.makeText(ScrapUpdAssetActivity.this,"网络未连接", Toast.LENGTH_SHORT);
            toast.setGravity(Gravity.CENTER, 0, 0);
            toast.show();
        }else{
            // 提示框
            dialog = new ProgressDialog(ScrapUpdAssetActivity.this);
            dialog.setTitle("提示");
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btnBack:
                Redirect.redirect(ScrapUpdAssetActivity.this, ScrapUpdActivity.class, bundle);
                break;
            case R.id.btnQuit:      //保存
                ...
                if(stoLists.size()==0){
                    Dialog dialog = new AlertDialog.Builder(ScrapUpdAssetActivity.this)
                            .setTitle("资产维修提示")
    ...
    View.OnKeyListener returnKey = new View.OnKeyListener() {
        ...
        public boolean onKey(View v, int keyCode, KeyEvent event) {
            if (keyCode == KeyEvent.KEYCODE_ENTER && event.getAction() == KeyEvent.ACTION_DOWN) {
                Redirect.hideIM(v,ScrapUpdAssetActivity.this);
